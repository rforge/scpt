...
RcppExport SEXP get_svar(SEXP locArray, SEXP DataArray, SEXP Dims){
    ...
    for(i = 0; i < K; i++){
        ...
        for (j = i; j < K; j++) {
            ...
            //temp = CH_D(loc(i,1), loc(j,1), loc(i,0), loc(j,0));
    ...
    for(i = 0; i < K; i++){
        for(j = i; j < K; j++){
            ...
            for(n = 0; n < N; n++){
                ...
                temp += sum/(2*((double) N))/(c_hat(i)*c_hat(j));
                //temp += sum/(2*((double) N));//modified 06/04/2014
            }
            rho(i,j) = temp;
            rho(j,i) = temp;
        }
